	...
	{#if showForm}
		<div class="mb-8 rounded-lg border border-zinc-800 bg-zinc-900/70 p-6">
			...
			<form method="POST" action="?/{formMode}" use:enhance={handleSubmit}>
				...
				<div class="mb-4">
					<label for="content" class="mb-1 block text-sm font-medium text-zinc-300"
				...
				<div class="mb-4 grid grid-cols-2 gap-4">
					...
					<div>
						<label for="published" class="mb-1 block text-sm font-medium text-zinc-300"
							>Status</label
						>
						<div
							class="flex items-center space-x-4 rounded-md border border-zinc-700 bg-zinc-800 p-2"
						>
					...
					{:else}
						{#each data.posts as post (post.id)}
							<tr class="border-b border-zinc-800">
								...
								<td class="px-4 py-3">
									<span
										class={`rounded-full px-2.5 py-0.5 text-xs font-medium ${post.published ? 'bg-green-900/20 text-green-400' : 'bg-amber-900/20 text-amber-400'}`}
									>
